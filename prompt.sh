...
if [ -n "${BASH_VERSION}" ]; then
    ...
    function custom_build_prompt {
        ...
        if [[ $is_a_git_repo == true ]]; then
            ...
            prompt+=$(enrich_append $has_stashes $omg_has_stashes_symbol "${red_on_white}")
            ...
            prompt="${prompt} ${white_on_red} ${white_on_blue}"
            if [[ $detached == true ]]; then
                ...
                if [[ $has_upstream == false ]]; then
                    prompt+=$(enrich_append true "-- ${omg_not_tracked_branch_symbol}  --  (${current_branch})" "${white_on_blue}")
                else
                    ...
                    if [[ $has_diverged == true ]]; then
                        ...
                        if [[ $commits_behind -gt 0 ]]; then
                            prompt+=$(enrich_append true "-${commits_behind} ${white_on_red}${omg_can_fast_forward_symbol}${white_on_blue} --" "${white_on_blue}")
                        fi
                        if [[ $commits_ahead -gt 0 ]]; then
                            prompt+=$(enrich_append true "-- ${white_on_red}${omg_should_push_symbol}${white_on_blue}  +${commits_ahead}" "${white_on_blue}")
                        fi
                        if [[ $commits_ahead == 0 && $commits_behind == 0 ]]; then
                            prompt+=$(enrich_append true " --   -- " "${white_on_blue}")
                        fi
                        
                    fi
                    prompt+=$(enrich_append true "(${current_branch} ${type_of_upstream} ${upstream//\/$current_branch/})" "${white_on_blue}")
                fi
            fi
            prompt+=$(enrich_append ${is_on_a_tag} "${omg_is_on_a_tag_symbol} ${tag_at_current_commit}" "${white_on_blue}")
